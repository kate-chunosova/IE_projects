...
class AddNewFeatures(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self,
        atemp_vs_hum=True,
        windspeed_vs_atemp=True,
        windspeed_atemp=True,
        windspeed_hum=True,
    ):
    ...
    def transform(self, X):
        ...
        try:
            ...
            if self.windspeed_vs_atemp:
                X.loc[:, "windspeed_vs_atemp"] = X.loc[:, "windspeed"] / (
                    X.loc[:, "atemp"] + 1
                )
            ...
            cols_error = list(
                set(["temp", "atemp", "hum", "windspeed"]) - set(X.columns)
            )
            raise KeyError(
                "[AddNewFeatures] DataFrame does not include the columns:", cols_error
            )
